package boatcraft.compatibility.redstoneflux
...
class EnergyCell(meta: Int) extends Block
{
	...
	private class Data extends IEnergyHandler
	{
		...
		override def extractEnergy(from: EnumFacing, maxExtract: Int, simulate: Boolean) =
			storage.extractEnergy(maxExtract, simulate)
		
		override def getEnergyStored(from: EnumFacing) = storage.getEnergyStored
		
		override def getMaxEnergyStored(from: EnumFacing) = storage.getMaxEnergyStored
	}*/
}
