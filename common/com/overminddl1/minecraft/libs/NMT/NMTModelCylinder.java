...
public class NMTModelCylinder extends NMTModelBase
{
	...
	public NMTModelBase create()
	{
		...
		for (int repeat = 0; repeat < (coneBase || coneTop ? 1 : 2); repeat++)
		{
			...
			{
				final float xSize = mirrorDir * MathHelper.sin(pi / segments * index * 2F + pi) * radius * sCur;
		...
		float uScale = 1.0F / renderer.textureWidth;
		float vScale = 1.0F / renderer.textureHeight;
		...
		float uStart = renderer.textureU * uScale;
		float vStart = renderer.textureV * vScale;
		...
		for (int index = 0; index < segments; index++)
		{
			...
			if (!coneBase && !coneTop)
			{
				vert4[0] = tempVerts[1 + index].setUV(uStart + uOffset + uWidth * index, vStart + vOffset + vCircle);
				vert4[1] = tempVerts[1 + index2].setUV(uStart + uOffset + uWidth * (index + 1), vStart + vOffset + vCircle);
				vert4[2] = tempVerts[1 + segments + index2].setUV(uStart + uOffset + uWidth * (index + 1), vStart + vOffset + vCircle + vHeight);
				vert4[3] = tempVerts[1 + segments + index].setUV(uStart + uOffset + uWidth * index, vStart + vOffset + vCircle + vHeight);
