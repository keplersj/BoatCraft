...
public class NMTModelTrapezoid extends NMTModelBase
{
	...
	{
		float x1 = xPos - (float) (width * (scale - 1) * 0.5);
		float y1 = yPos - (float) (height * (scale - 1) * 0.5);
		float z1 = zPos - (float) (depth * (scale - 1) * 0.5);
		float x2 = x1 + width * scale;
		float y2 = y1 + height * scale;
		float z2 = z1 + depth * scale;
